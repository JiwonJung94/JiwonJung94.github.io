...
def render_book(book_dir: str | Path, out_dir: str | Path, book_id: str | None=None, css: str | None=None, site_title=None, site_url: str | None=None, site_default_locale: str | None=None, views_url: str | None=None, verification=None, analytics_cf=None) -> Book:
    ...
    for locale in book.locales:
        ...
        for (idx, page) in enumerate(resolved):
            ...
            resume_json = json.dumps({'book': book.book_id, 'slug': page.out_name[:-5], 'lang': locale, 'title': page.title, 'bookTitle': book.title(locale)}, ensure_ascii=False)
            locale_options = []
            ...
            html = page_tmpl.render(book_title=book.title(locale), page_title=page.title, lang=locale, is_fallback=page.is_fallback, default_locale=book.default_locale, content_html=content_html, nav=[{**c, 'active': c['key'] == (page.chapter, page.section, page.slug), 'children': [{**ch, 'active': ch['key'] == (page.chapter, page.section, page.slug)} for ch in c['children']]} for c in nav], locale_options=locale_options, prev={'href': prev_p.out_name, 'label': prev_p.title} if prev_p else None, next={'href': next_p.out_name, 'label': next_p.title} if next_p else None, site_home='/home/', site_name=site_name, copyright=copyright_text, contact_email=contact_email, meta_description=meta_description, robots_noindex=robots_noindex, canonical=canonical, hreflang=hreflang, hreflang_xdefault=hreflang_xdefault, og=og, views_url=views_url, views_json=views_json, resume_json=resume_json, jsonld=jsonld, verification=verification, analytics_cf=analytics_cf, css=css, js=js)
        ...
        land_views_json = json.dumps({'url': views_url, 'book': book.book_id, 'lang': locale, 'scope': 'book'}, ensure_ascii=False) if views_url else None
        landing_html = landing_tmpl.render(lang=locale, book_title=book.title(locale), author=author, description=land_desc, meta_description=land_desc or '', cover=land_cover, cover_abs=land_cover_abs, toc=nav, first_href=resolved[0].out_name if resolved else 'index.html', read_label=READ_LABEL.get(locale, READ_LABEL['en']), locale_options=land_options, site_home='/home/', site_name=site_name, copyright=copyright_text, contact_email=contact_email, canonical=land_canonical, hreflang=land_hreflang, hreflang_xdefault=land_xdefault, jsonld=land_jsonld, verification=verification, analytics_cf=analytics_cf, views_url=views_url, views_json=land_views_json, css=css, js=js)
